...
# NOTE: despite the "_latent" filename, the version of this wrapper found in
# source had --brain_path pointing at a T1-percentile GWAS directory, not a
# latent-dimension GWAS directory -- filename and content disagreed. Rather
# than guess which was right, --brain_path and --out_file below are
# placeholders; set LATENT_GWAS_DIR to your actual VAE-latent-dimension GWAS
# output directory (e.g. the latent_dimensions_HHregressed equivalent of
# 05_gwas/gwas_VAE.sh's --out) before running this job.
LATENT_GWAS_DIR="<LATENT_GWAS_DIR>"
...
${BASE_DIR}/bruna/vcf_gwas_pa/protein_example/out_tar/cis_subsets_r2_001" \
  --brain_path="${LATENT_GWAS_DIR}" \
  --out_file="${BASE_DIR}/bruna/vcf_gwas_pa/MR_experiment/cis_scripts/shriya_T1/MR_latent_dimensions.tsv"
