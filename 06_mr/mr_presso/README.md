...
Manifests of protein-outcome pairs to run MR-PRESSO on (whitespace-separated, 2 columns, no header: `<gene_symbol> <outcome_name>`). **`mrpresso_pairs_2.txt`** (20 rows) is the file `run_MR_PRESSO.sh` actually reads. `mrpresso_pairs.txt` (37 rows) is kept as the other candidate manifest -- see `docs/REVIEW_REQUIRED.md` for the unresolved question of which one was used for the reported run.
...
- **Output:** under `<outcome_dir>/${RESULTS_SUBDIR}/` (`RESULTS_SUBDIR` from the `$MRPRESSO_RESULTS_SUBDIR` environment variable -- **must be set**, see `docs/REVIEW_REQUIRED.md`): `<pair>_main_results.csv`, `_global_test.csv`, `_outlier_test.csv`, `_distortion_test.csv`, `_outlier_snps.csv` (if any), plus `.rds` intermediates.
